...
const loadStatsPage = () => {
	let wins = 0, losses = 0
	let rows = document.getElementById('stats_table').rows
	for (let i = 2, row; row = rows[i]; i++) {
    if(row.cells[2].innerText > row.cells[3].innerText){
			//home wins
			row.cells[4].innerText = 'CU Boulder'
			wins++
		}else{
			//away wins
...
const loadPlayersPage = () => {
	...
	for(i in players){
		...
		let linkText = document.createTextNode(players[p].name)
		a.appendChild(linkText)
		a.title = "PLAYER"
		a.href = "#"
		a.style = 'display: block'
		a.onclick = () => switchPlayers(p)
		playerSelector.appendChild(a)
	}
}
